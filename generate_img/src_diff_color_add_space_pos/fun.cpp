...
IplImage* GenData::add_no_backgrand(){
    ...
    int rand_red = rand()%50;
    int rand_grey = rand()%50;
    int rand_blue = rand()%50;
	cvSet(pimg, cvScalar(rand_red, rand_grey, rand_blue));
    int red_color = rand()%50 + 216;
    int green_color = rand()%50 + 216;
    int blue_color = rand()%50 + 216;
 	//set image value
    ...
    if ((int)pixels[i].color < 50) {
        pdata[0]=(uchar)(red_color);
        pdata[1]=(uchar)(green_color);
        pdata[2]=(uchar)(blue_color);
    }
...
IplImage* GenData::add_fushion_backgrand(){
  ...
  int start_x=rand()%(pimg_bgimg->width-width-5); //lx modified, to make more resduial
  int start_y=rand()%(pimg_bgimg->height-height-5);
  ...
  cvAddWeighted(pimg,alpha,pimg_bgimg,beta,gamma,pimg);
...
IplImage* GenData::add_finegrand_forgrand_backgrand(){
  if(foreground_background_imgs.empty()){
    LOG(WARNING)<<"foreground_background_imgs are empty.";
    return NULL;
  	//return add_no_backgrand();
  ...
  uchar* ptestdata=(uchar*)(pimg->imageData);
  while (abs(red_color * red_color + green_color * green_color + blue_color * blue_color - 
        (int)ptestdata[0] * (int)ptestdata[0] - (int)ptestdata[1] * (int)ptestdata[1] -
        (int)ptestdata[2] * (int)ptestdata[2]) < 10000) {
        red_color = rand()%150;
        green_color = rand()%150;
        blue_color = rand()%150;
  }
  ...
  for(int i=0; i<pixels.size(); i++){
   ...
   uchar* pdata=(uchar*)(pimg->imageData+pixels[i].py*pimg->widthStep+3*pixels[i].px);
   //std::cout << (int)pixels[i].color << " , ";
  if (abs(red_color * red_color + green_color * green_color + blue_color * blue_color - 
        (int)ptestdata[0] * (int)ptestdata[0] - (int)ptestdata[1] * (int)ptestdata[1] -
        (int)ptestdata[2] * (int)ptestdata[2]) < 3000) {
        LOG(WARNING) << "words' color is similar to the background";
        cvReleaseImage(&pimg);
        return NULL;
   }
   ...
   if ((int)pixels[i].color < 50) {
        ...
        pdata[1]=(uchar)(green_color);
        pdata[2]=(uchar)(blue_color);
        /*pdata[0]=(uchar)(red_color * 0.5 + pdata[0] * 0.5);
        pdata[1]=(uchar)(green_color * 0.5 + pdata[1] * 0.5);
        pdata[2]=(uchar)(blue_color * 0.5 + pdata[2] * 0.5);*/
   }
 }
  cvResetImageROI(pimg_bgimg);
  return pimg;   
...
IplImage* GenData::add_backgrand(){
  ...
  if(ticket<no_backgrand_problity){
     ...
     return add_finegrand_forgrand_backgrand();
   }
  else {
...
bool GenData::serialize(string& name)
{
   ...
   for(int i=0;i<v.size();i++){
    ...
    int new_hei = 48;
    std::cout << "raw_size: " << pimg->height << " " << pimg->width << " " << std::endl;
	...
	for (int i = 0; i < Sentence.size(); ++i)
	{
		...
		//output retangle 
        std::cout << Sentence[i].left << " " << Sentence[i].width << " " << Sentence[i].top << " " << Sentence[i].height << " " << scale << std::endl;
		int x1=(int)(Sentence[i].left * scale);
		int x2=(int)((Sentence[i].left+Sentence[i].width) * scale);
		int y1=(int)(Sentence[i].top * scale);
		int y2=(int)((Sentence[i].top+Sentence[i].height) * scale);
