...
float g_dect_time = 0;
float g_recg_time = 0;
float g_recg_score_thres = 0.44;

//global vairables used by detection
linegenerator::line_gen_para g_line_param;
...
int dect_recg_ocr_rst(std::string file_name, std::string save_dir) {
    ...
    rcnn_loc.init_model_para_v3(g_fcn_model, g_seq_model._m_recog_model_attr, g_line_param);
    ...
    recg_result.filter_word_with_low_prob(g_recg_score_thres);
...
int main(int argc, char* argv[]){
    if (argc < 21){//6->6+13+1+1
        std::cerr << "./test-main [detection_model_file] [rec_model_file] [img_path]" 
            << " [res_path]"
            << " [dect_dir:0-without dir dect/1-with dir dect]" 
            << " [line_para: min_score good_score max_score isolate_score"
            << " lambda if_all_same_angle  must_horizontal max_main_angle_diff"
            << " prefer_ratio expand_ratio_x expand_ratio_y" 
            << " line_generation_method line_registration_method pair_generation_method]"
            << " [recg_para: recg_score_thres]" << std::endl;
        return -1;
    }
    ...
    int dect_dir = atoi(argv[5]);
    //line_gen_para
    sscanf(argv[6], "%f", &g_line_param.min_score_thres);
    sscanf(argv[7], "%f", &g_line_param.good_score_thres);
    sscanf(argv[8], "%f", &g_line_param.short_path_max_score_thres);
    sscanf(argv[9], "%f", &g_line_param.isolate_box_max_score_thres);
    sscanf(argv[10], "%f", &g_line_param.lambda);
    sscanf(argv[11], "%d", &g_line_param.if_all_same_angle);
    sscanf(argv[12], "%d", &g_line_param.must_horizontal);
    sscanf(argv[13], "%d", &g_line_param.max_main_angle_diff);
    sscanf(argv[14], "%f", &g_line_param.prefer_ratio);
    sscanf(argv[15], "%f", &g_line_param.expand_ratio_x);
    sscanf(argv[16], "%f", &g_line_param.expand_ratio_y);
    sscanf(argv[17], "%d", &g_line_param.line_generation_method);
    sscanf(argv[18], "%d", &g_line_param.line_registration_method);
    sscanf(argv[19], "%d", &g_line_param.pair_generation_method);
    //recg_para
    sscanf(argv[20], "%f", &g_recg_score_thres);
    ...
    if (argc >= 22) {
        snprintf(g_pdebug_title, 256, "%s", argv[21]);
        printf("Debug Title:%s\n", g_pdebug_title);
    }
    g_debug_line_index = -1;
    if (argc >= 23) {
        g_debug_line_index = atoi(argv[22]);
...
{    
    ...
    snprintf(fn_extrainfo_name, 256, "%s/fcn_data/fcn_extra_info_vgg9conv.prototxt", model_dir);
    // init densebox params
    // rcnnchar::DenseBoxParam g_dense_box_param;
    int file_descriptor = open(fn_extrainfo_name, O_RDONLY);
    if (file_descriptor < 0)
    {
        fprintf(stderr, "Can not load extra info model\n");
        return 0;
    }
    ...
    {
        fprintf(stderr, "Fail to parse file!\n");
        return 0;
    } 
    ...
    ret = dect_model->init(str_model_name_fcn_detector_general_proto, 
            str_model_name_fcn_detector_general_weight, dense_box_param);
    if (0 != ret)
    {
        return -1;
    }
