    ...
    while (local_loop_num){
        ...
        for (unsigned int i = 0; i < g_img_name_v.size(); ++i){
            ...
            for (int i_line = 0; i_line < in_line_dect_vec.size(); ++i_line){
                ...
                g_seq_model_eng.recognize_image_att(c_img, c_in_line_dect_vec,
                        c_out_line_vec);
...
int main(int argc, char* argv[]){
    if (argc != 6){
        std::cerr << "./test-main-new detection_mode rec_model"
            << "img_dir res_dir loop_num" << std::endl;
        std::cout << "*Note that paddle predict does not support multi-thread" << std::endl;
        return -1;
    }
    ...
    g_seq_model.init_att(recognition_model_path, esp_model_conf_name_en, inter_lm_trie_name_en,
                     inter_lm_target_name_en, table_name_en);

    g_seq_model_eng.init_model(g_seq_model._m_eng_search_recog_eng,
            g_seq_model._m_inter_lm_eng, g_seq_model._m_recog_model_eng_att); 
