...
int dect_recg_ocr_rst(std::string file_name, std::string save_dir) {
    ...
    g_seq_model_ch_batch.recognize_image_batch_only_eng(vec_batch_info,
    ...
    for(unsigned int i = 0; i < out_batch_vec.size(); i++){
        std::cout<<"out_batch_vec:"<<out_batch_vec[i].lineStr_<<std::endl;
    }
...
int main(int argc, char* argv[]){
    ...
    bool eng_paddle_flag = true;
    bool eng_gpu_flag = true;
    char eng_model_path[256];
    snprintf(eng_model_path, 256, "./data/model_eng/gru_eng_model");
    ret_val = nsseqocrchn::init_model_paddle_deepcnn_eng_gpu(recognition_model_path,
        chn_paddle_flag, chn_gpu_flag,
        eng_paddle_flag, eng_gpu_flag,
        g_seq_model, g_seq_model_ch_batch, eng_model_path);
    if (ret_val != 0) {
