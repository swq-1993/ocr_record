...
static void concat_words(SSeqLEngLineInfor one_line, SSeqLEngLineInfor & one_line_concat,
                std::vector<SSeqLEngWordInfor> & all_words) {
    one_line_concat.lineStr_ = "";
    for (int i = 0; i < one_line.wordVec_.size(); ++i) {
        if (i == 0){
            if (one_line.wordVec_[i].wordStr_.size() >= 1){
                all_words.push_back(one_line.wordVec_[i]);
            }
        }else{
            if (one_line.wordVec_[i].wordStr_ != "" && one_line.wordVec_[i].wordStr_ != "i"){
                all_words.push_back(one_line.wordVec_[i]);
            }
        }
    }
//    std::cout << "the all_words size is: " << all_words.size() << std::endl;
    for (int j = 0; j< all_words.size(); j++){
        one_line_concat.lineStr_ += all_words[j].wordStr_;
        if (j != (all_words.size() - 1)){
            one_line_concat.lineStr_ += " ";
        }
    }
}

static void save_whole_line(std::string saveName, SSeqLEngLineInfor & one_line, std::string format){
    SSeqLEngLineInfor one_line_concat;
    std::vector<SSeqLEngWordInfor> all_words;
    concat_words(one_line, one_line_concat, all_words);

    FILE *fp_retfile = NULL;
    fp_retfile = fopen(saveName.c_str(), format.c_str());
    if (fp_retfile == NULL){
        std::cerr << "Can not open the file " << saveName << std::endl;
        return;
    }
    if (one_line_concat.lineStr_.size() > 1){
        fprintf(fp_retfile, "%d\t%d\t%d\t%d\t%s\n", one_line.left_, one_line.top_,
                        one_line.wid_, one_line.hei_, one_line_concat.lineStr_.c_str());
        /*for (int i = 0; i < all_words.size(); i++){
            if(i != (all_words.size() - 1)){
                fprintf(fp_retfile, "%d\t%d\t%d\t%d\t%s\t", all_words[i].left_,
                all_words[i].top_, all_words[i].wid_, all_words[i].hei_, all_words[i].wordStr_.c_str());
            }else{
                fprintf(fp_retfile, "%d\t%d\t%d\t%d\t%s\t\n", all_words[i].left_,
                all_words[i].top_, all_words[i].wid_, all_words[i].hei_, all_words[i].wordStr_.c_str());
            }
...
static void save_whole_line(std::string save_name, std::vector<SSeqLEngLineInfor> & line_vec) {
    FILE *fp_retfile = NULL;
    std::string format_org = "w";
    fp_retfile = fopen(save_name.c_str(), format_org.c_str());
    if (fp_retfile == NULL){
        std::cerr << "Can not open the file " << save_name << std::endl;
        return;
    }
    fprintf(fp_retfile, "%s\t\n", save_name.c_str());
    fclose(fp_retfile);
    fp_retfile = NULL;

    for (int i = 0; i < line_vec.size(); i++){
        if (i == 0){
            save_whole_line(save_name, line_vec[i], "a");
        } else {
            save_whole_line(save_name, line_vec[i], "a");
        }
    }
...
void* core_func(void *argv){
    ...
    while (local_loop_num){
        ...
        for (unsigned int i = 0; i < g_img_name_v.size(); ++i){
            ...
            linegenerator::text_detection_v2(src_img, g_cdnn_model, text_line_detection_results,
                            detected_boxes_post, g_para);
            ...
            save_whole_line(recog_name, out_line_vec);
...
int main(int argc, char* argv[]){
    ...
    g_para.set_para(0.15, 0.4, 0.7, 0.6, 0.1, 1, 0, 1, 0.75f, 0.2f, 0.2f, 0, 0, true);
    g_para.center_dist_thresh2 = 5.0f;
    g_para.max_scale_diff = 3.0f;
    g_para.max_line_dist = 1.0f;
    g_para.size_dist_thresh = 0.8f;
    g_para.angle_horizontal_thres = 0.2618;
